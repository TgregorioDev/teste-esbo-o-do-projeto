...
test.describe('A SC criada nasce no estado e no dono corretos (CT-E2E-01-H)', () => {
  test('@destrutivo estado inicial e responsável deveriam refletir uma etapa de trabalho do solicitante', async ({
    ...
    // aparecer". Ver `MinhasSolicitacoesPage`: a varredura é DECRESCENTE por `processInstanceId`
    // porque a listagem pagina por cursor e, em ordem crescente, uma SC recém-criada nunca é
    // alcançada — foi o que produzia `Received: null` aqui sem dizer a causa.
