 ...
 *
 * Massa própria desde 11/09/2026: a fixture `solicitacaoNoPool` cria uma SC por API (`POST /start`
 * com `targetState: 0`, `utils/massa-sc-api.js`) e espera, no servidor, ela cair no pool da
 * Validação do Gestor. Antes disto o teste dependia de haver tarefa de outra pessoa no pool no
 * momento da execução. Se o painel ainda assim anunciar pool vazio, falha via `faltaPreCondicao`
 * em vez de timeout opaco.
 *
...
test.describe('Central de Tarefas — assumir tarefa do pool (CT-TSK-02-H) @destrutivo', () => {
  test('assumir a primeira tarefa disponível de um grupo do pool deve movê-la para "Tarefas a concluir"', async ({
    ...
    if (poolAntes.total === 0) {
      faltaPreCondicao(
        `(ambiente): o Resumo de Tarefas anuncia "Tarefas em pool (0)" mesmo com a SC de massa ` +
          `${solicitacaoNoPool.numeroProcesso} já no pool da Validação do Gestor segundo o servidor. ` +
          'O painel-resumo trabalha com cache e já foi medido desatualizado (25/08/2026) — não há ' +
          'tarefa a assumir por esta tela agora. Corrigido em 11/09/2026: a versão anterior desta ' +
          'mensagem dizia que a automação não conseguia criar massa de pool; com `targetState: 0` ' +
          'consegue, e é o que a fixture `solicitacaoNoPool` faz.',
      );
    }
