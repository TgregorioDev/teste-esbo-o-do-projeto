...
| **Cancelamento de SC integrada** 🔴 | cancelamento da SC | *(medido 11/09/2026 — enviado ao desenvolvedor, aguardando retorno)* SC que já existe no Protheus (`numSolCompra` preenchido) e ainda não tem cotação **não pode ser cancelada**, por nenhum dos dois endpoints: o `beforeCancelProcess` de `wf_solicitacao_compras` pede ao ERP a exclusão das cotações, recebe 404 *"Não foram encontradas contações para exclusão"* e aborta com `BPMBeforeCancelException`. Entre "Grava SC e Anexos" e a cotação, nem o solicitante cancela. Na limpeza das 14 SCs órfãs de 10/09, as 5 sem `numSolCompra` cancelaram e as 9 com número foram recusadas. Teste: `tests/e2e/compras/cancelamento-sc-integrada.spec.js` |
