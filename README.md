...
Como funciona: a fixture escreve `playwright/.massa/criados.jsonl` (`utils/livro-razao.js`) no instante da criação — não
...
| **Grade da Gerência de Compras** 🔴 | gerência de compras | *(remedido 11/09/2026 — o registro antigo, "a tabela nunca renderiza dados", não vale mais)* as abas Atribuir e Transferir listam processo **CANCELADO** como se esperasse atribuição: as 31 linhas das duas abas eram de solicitações encerradas. `ds_getSolicsGerenciaCompras` não filtra instância com `END_DATE`. Teste `@bug` em `atribuicao-comprador.spec.js` |
| **FSWTBC-5118 — alçada em pool** 🔴 | Aprovação de Alçadas | *(reproduzido em toda execução desde 09/09/2026)* há tarefa aberta de *Aprovação de Alçadas* atribuída ao grupo `G.P.Requisicao_de_Compras_Validacao_Alcadas`, e não a um aprovador nominal (ex.: instância 95437). Alçada em pool é consenso para todo o grupo — o comportamento que o FSWTBC-3957 descreve como indevido. Teste `@bug` em `tests/api/alcada-solicitacao-compras.spec.js` |
...
| **Cancelamento de SC integrada** ✅ corrigido | cancelamento da SC | *(medido 11/09/2026 de manhã; **corrigido no mesmo dia** — remedido à tarde, SC 96501 cancelada e o teste deixou de ser `@bug`)* SC que já existe no Protheus (`numSolCompra` preenchido) e ainda não tem cotação **não pode ser cancelada**, por nenhum dos dois endpoints: o `beforeCancelProcess` de `wf_solicitacao_compras` pede ao ERP a exclusão das cotações, recebe 404 *"Não foram encontradas contações para exclusão"* e aborta com `BPMBeforeCancelException`. Entre "Grava SC e Anexos" e a cotação, nem o solicitante cancela. Na limpeza das 14 SCs órfãs de 10/09, as 5 sem `numSolCompra` cancelaram e as 9 com número foram recusadas. Teste: `tests/e2e/compras/cancelamento-sc-integrada.spec.js` |
